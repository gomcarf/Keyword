...
| **구분** | **프로세스 (Process)** | **스레드 (Thread)** | **비유 (공장 안의 상황)** |
| --- | --- | --- | --- |
| **개념** | 실행 중인 프로그램 (독립된 작업 단위) | 프로세스 내의 실행 흐름 (실행의 주체) | **프로세스** = 독립된 공장 건물
**스레드** = 공장 안에서 일하는 일꾼 |
| **메모리 구조** | Code, Data, Heap, Stack 모두 독립적 | Code, Data, Heap은 **공유**
Stack만 **독립적** | **공유** = 공장의 공동 작업대, 도구함
**독립** = 일꾼 개인의 수첩 |
| **생성/전환 비용** | 큼 (메모리 할당 및 복사 필요) | 작음 (자원을 공유하므로 빠름) | 신설 공장을 짓는 것 vs
기존 공장에 일꾼을 새로 고용하는 것 |
| **장애 영향** | 다른 프로세스에 영향 없음 | 한 스레드가 죽으면 프로세스 전체 종료 | 옆 공장이 불타도 우리 공장은 안전함 vs
동료 일꾼 한 명이 폭탄을 터뜨리면 공장 전체가 날아감 |
| **통신 방식** | IPC(파이프, 소켓 등) 필요 (복잡함) | 공유 메모리를 통해 직접 통신 (간단함) | 다른 공장과 전화나 우편으로 소통 vs
같은 방 안에서 말로 바로 소통 |

<img src="Images/image2.png" width=700></img>
